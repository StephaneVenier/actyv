...
create or replace function public.compute_session_mastery_value(
  p_mastery_measurement_type text,
  p_mastery_unit text,
  p_block_type text,
  p_completed_sets integer,
  p_total_reps numeric,
...
begin
  ...
  completed_rows as (
    select
      ...
      greatest(coalesce(raw_rows.actual_charge_kg, raw_rows.planned_charge_kg, 0), 0) as charge_value,
  ...
  aggregated_blocks as (
    select
      ...
      sum(case when completed_rows.block_type = 'reps' then completed_rows.reps_value * completed_rows.charge_value else 0 end) as total_volume_kg
    from completed_rows
    group by
      completed_rows.block_id,
      completed_rows.block_name,
  ...
  candidate_links as (
    select
      ...
      mastery.unit,
      case
        when aggregated_blocks.block_uuid is not null and mel.exercise_id = aggregated_blocks.block_uuid then 1
        when public.normalize_mastery_exercise_key(mel.exercise_key) = aggregated_blocks.normalized_block_key then 2
        when public.normalize_mastery_exercise_key(mel.exercise_name) = aggregated_blocks.normalized_block_key then 3
        else 99
    ...
    join public.mastery_exercise_links mel
      on (
        (aggregated_blocks.block_uuid is not null and mel.exercise_id = aggregated_blocks.block_uuid)
  ...
  resolved_blocks as (
    select
      ...
      case resolved_links.match_priority
        ...
        when 2 then 'exercise_key'
        when 3 then 'exercise_name'
        else null
      end as matched_by,
...
revoke all on function public.normalize_mastery_exercise_key(text) from public;
revoke all on function public.normalize_mastery_exercise_key(text) from anon;
revoke all on function public.normalize_mastery_exercise_key(text) from authenticated;

revoke all on function public.compute_session_mastery_value(text, text, text, integer, numeric, numeric, numeric, numeric) from public;
revoke all on function public.compute_session_mastery_value(text, text, text, integer, numeric, numeric, numeric, numeric) from anon;
revoke all on function public.compute_session_mastery_value(text, text, text, integer, numeric, numeric, numeric, numeric) from authenticated;
