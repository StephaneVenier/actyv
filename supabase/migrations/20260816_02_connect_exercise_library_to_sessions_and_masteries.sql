...
create index if not exists training_session_blocks_exercise_id_idx
  on public.training_session_blocks (exercise_id);
...
set exercise_id = el.id
from public.exercise_library el
where public.normalize_mastery_exercise_key(coalesce(nullif(mel.exercise_name, ''), mel.exercise_key)) = el.slug
      ...
      max(
        case
          when completed_rows.exercise_id ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$'
            then completed_rows.exercise_id::uuid
          else null
        end
      ) as exercise_uuid,
